...
class DashboardAdminController extends Controller
{
    public function index()
    {
        ...
        return view('Approval-app.HelpDesk.index', compact(
            'overviewStats',
            'departmentStats', 
            'topRequesters',
            'recentActivities',
    ...
    private function getFinancialSummary()
    {
        ...
        // Menghitung Total Budget Diminta: SUM dari nominal_sblm_revisi (jika ada) atau nominal_pengajuan (untuk semua status)
        $totalRequested = Pengajuan::all()->sum(function($pengajuan) {
            // Gunakan nominal_sblm_revisi jika ada, jika tidak, gunakan nominal_pengajuan.
            // Ini mencakup semua pengajuan terlepas dari statusnya (sesuai definisi tooltip)
            return $pengajuan->nominal_sblm_revisi ?: $pengajuan->nominal_pengajuan;
        });

        // Total Budget Disetujui (dari yang Completed)
        $totalApproved = Pengajuan::where('status_pengajuan', 'completed')->sum('nominal_pengajuan');

        return [
            // Perbarui ini
            'total_budget_requested' => $totalRequested, 
            
            // Perbarui ini agar hanya menghitung nominal yang disetujui/final (completed)
            'total_budget_approved' => $totalApproved, 
