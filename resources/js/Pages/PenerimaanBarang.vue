  ...
  <AppLayout title="Riwayat Aktivitas">
    <div class="space-y-6">
      ...
      <div v-if="showDetailModal"
        class="fixed inset-0 bg-gray-600 bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-[9999]" style="background-color: rgba(43, 51, 63, 0.67);">
      ...
      <div v-if="showQRCodeModal"
        ...
        <div class="bg-white rounded-lg max-w-4xl w-full mx-4 max-h-[80vh] overflow-y-auto">
          <div class="p-6">
            <!-- Header Modal QR -->
            <div class="flex justify-between items-center mb-6">
              <h3 class="text-lg font-semibold text-gray-900">Label QR Code - {{ selectedShipment?.noSuratJalan }}</h3>
...
const generateQRCodes = () => {
  ...
  selectedShipment.value.items.forEach((item, index) => {
    ...
    if (canvas) {
      ...
      try {
        QRCode.toCanvas(canvas, item.qrCode, {
          width: 180,
          height: 180,
          margin: 1,
          ...
          <div class="label-container">
            ...
            <div class="content">
              <div class="info-section">
                <div class="info-row">
                  ...
                  <div class="info-value">: ${item.namaMaterial.length > 20 ? item.namaMaterial.substring(0, 20) + '...' : item.namaMaterial}</div>
                </div>
                <div class="info-row">
                  <div class="info-label">Kode Barang</div>
                  <div class="info-value">: ${item.kodeItem}</div>
                </div>
                <div class="info-row">
                  <div class="info-label">No Lot</div>
                  <div class="info-value">: ${item.batchLot}</div>
                </div>
                <div class="info-row">
                  <div class="info-label">Supplier</div>
                  <div class="info-value">: ${selectedShipment.value.supplier.length > 18 ? selectedShipment.value.supplier.substring(0, 18) + '...' : selectedShipment.value.supplier}</div>
                </div>
                <div class="info-row">
                  <div class="info-label">Jmlh Barang</div>
                  <div class="info-value">: ${item.qtyUnit} Pcs</div>
                </div>
                <div class="info-row">
                  <div class="info-label"></div>
                  <div class="info-value">: ${item.qtyWadah || '-'} box</div>
                </div>
                <div class="info-row">
                  <div class="info-label">Tgl Datang</div>
                  <div class="info-value">: ${formatDateOnly(selectedShipment.value.tanggalTerima)}</div>
                </div>
                <div class="info-row">
                  <div class="info-label">Exp. Date</div>
                  <div class="info-value">: ${item.expDate}</div>
                </div>
                <div class="info-row">
            ...
            <div class="footer">
              <div class="footer-row">
                <span></span>
                <span class="footer-value">QL1001-01</span>
              </div>
              <div class="footer-row">
                <span></span>
                <span class="footer-value">Rev. 02</span>
              </div>
              <div class="footer-row">
                <span></span>
                <span class="footer-value">Logistik</span>
              </div>
            </div>
          </div>
  ...
  if (selectedShipment.value?.items?.length > 0) {
    ...
    selectedShipment.value.items.forEach((item, index) => {
      ...
      if (canvas) {
        const qrDataURL = canvas.toDataURL('image/png')
        labelsHTML += `
          <div class="label-container" style="${index > 0 ? 'page-break-before: always;' : ''}">
            <div class="header">
              <div class="company-name">GONDOWANGI</div>
              <div class="status-box">KARANTINA</div>
            </div>
