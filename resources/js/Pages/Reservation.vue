<template>
      ...
      <div class="p-6 border-b border-gray-200">
        <div class="flex flex-col lg:flex-row gap-4 mb-4">
          ...
          >
            ...
            Filter
            <span v-if="activeFiltersCount > 0" class="bg-blue-500 text-white text-xs px-2 py-0.5 rounded-full">{{ activeFiltersCount }}</span>
          </button>
        ...
        <div v-if="showFilterPanel" class="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div>
              ...
              <select v-model="filters.category" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700 bg-white">
            ...
            <div>
              ...
              <select v-model="filters.status" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700 bg-white">
            ...
            <div>
              ...
              <div class="flex gap-2">
                <input v-model="filters.dateFrom" type="date" class="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700 bg-white text-sm">
                <input v-model="filters.dateTo" type="date" class="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700 bg-white text-sm">
              </div>
            </div>
          </div>
          ...
          <div class="flex justify-between items-center">
            ...
            <div class="flex gap-2">
              ...
              <button @click="showFilterPanel = false" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm">
                Terapkan
              </button>
            </div>
          </div>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">No Reservasi</th>
                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Kategori</th>
                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tanggal Permintaan</th>
                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Detail Info</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-for="request in filteredRequests" :key="request.id" class="hover:bg-gray-50 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">{{ request.noReservasi }}</td>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ formatDateTime(request.tanggalPermintaan) }}</td>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div class="flex space-x-2">
                    <button @click="viewDetail(request)" class="bg-blue-50 text-blue-700 hover:bg-blue-100 px-3 py-1.5 rounded text-xs font-medium transition-colors">
                      Detail
                    </button>
                    <button @click="printForm(request)" class="bg-green-50 text-green-700 hover:bg-green-100 px-3 py-1.5 rounded text-xs font-medium transition-colors">
                      Cetak
                    </button>
                    <button v-if="request.status === 'Submitted'" @click="approveRequest(request)" class="bg-emerald-50 text-emerald-700 hover:bg-emerald-100 px-3 py-1.5 rounded text-xs font-medium transition-colors">
                      Approve
                    </button>
                    <button v-if="request.status === 'Submitted'" @click="rejectRequest(request)" class="bg-red-50 text-red-700 hover:bg-red-100 px-3 py-1.5 rounded text-xs font-medium transition-colors">
                      Reject
                    </button>
                  </div>
                </td>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-[9999]">
          ...
          <div v-if="selectedCategory" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input v-model="formData.noReservasi" type="text" readonly class="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-700">
              </div>

              <!-- Tanggal Permintaan (always) -->
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">
                  {{ selectedCategory === 'packaging' || selectedCategory === 'add' ? 'Tanggal & Jam Permintaan' : 'Tanggal Permintaan' }}
                </label>
                <input v-model="formData.tanggalPermintaan" type="datetime-local" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-gray-700">
              </div>
              ...
              <div v-if="selectedCategory === 'foh-rs'">
                ...
                <textarea v-model="formData.alasanReservasi" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-gray-700" placeholder="Jelaskan alasan reservasi..."></textarea>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div v-if="selectedCategory === 'foh-rs'">
                ...
                <select v-model="formData.departemen" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-gray-700">
              ...
              <div v-if="selectedCategory === 'packaging' || selectedCategory === 'add'">
                ...
                <input v-model="formData.namaProduk" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-gray-700">
              </div>
              ...
              <div v-if="selectedCategory === 'raw-material'">
                ...
                <input v-model="formData.kodeProduk" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-gray-700">
              </div>

              <!-- Packaging & ADD: No Bets Filling -->
              <div v-if="selectedCategory === 'packaging' || selectedCategory === 'add'">
                <label class="block text-sm font-medium text-gray-700 mb-1">No Bets Filling / Mixing</label>
                <input v-model="formData.noBetsFilling" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-gray-700">
              </div>

              <!-- Raw Material: No Bets -->
              <div v-if="selectedCategory === 'raw-material'">
                <label class="block text-sm font-medium text-gray-700 mb-1">No Bets Ekstrak / Mixing</label>
                <input v-model="formData.noBets" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-gray-700">
              </div>

              <!-- Raw Material: Besar Bets -->
              <div v-if="selectedCategory === 'raw-material'">
                <label class="block text-sm font-medium text-gray-700 mb-1">Besar Bets (Kg)</label>
                <input v-model="formData.besarBets" type="number" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-gray-700">
              </div>
            </div>
            ...
            <div class="border-t border-gray-200 pt-6">
              <div class="flex justify-between items-center mb-4">
                ...
                <button @click="addNewItem" class="bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded text-sm font-medium transition-colors">
                  + Tambah Baris
                </button>
              </div>
              ...
              <div class="overflow-x-auto border border-gray-200 rounded-lg">
                <table class="min-w-full divide-y divide-gray-200">
                  <thead class="bg-gray-50">
                    <tr>
                      <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap w-16">No</th>
                      
                      <!-- FOH & RS Headers -->
                      <template v-if="selectedCategory === 'foh-rs'">
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Kode Item</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Keterangan</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Qty</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">UoM</th>
                      </template>

                      <!-- Packaging Headers -->
                      <template v-if="selectedCategory === 'packaging'">
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Nama Material</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Kode PM</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Jumlah Permintaan</th>
                      </template>

                      <!-- Raw Material Headers -->
                      <template v-if="selectedCategory === 'raw-material'">
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Kode Bahan</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Nama Bahan</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Jumlah Kebutuhan</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Jumlah Kirim</th>
                      </template>
                      ...
                      <template v-if="selectedCategory === 'add'">
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Nama Material</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Kode PM</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Alasan Penambahan</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap">Jumlah Permintaan</th>
                      </template>

                      <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase whitespace-nowrap w-24">Aksi</th>
                    </tr>
                  </thead>
                    ...
                    <tr v-for="(item, index) in formData.items" :key="index">
                      ...
                      <template v-if="selectedCategory === 'foh-rs'">
                        <td class="px-4 py-3 whitespace-nowrap">
                          <input v-model="item.kodeItem" type="text" class="w-full min-w-[120px] text-sm border border-gray-300 rounded px-2 py-1 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">
                          <input v-model="item.keterangan" type="text" class="w-full min-w-[150px] text-sm border border-gray-300 rounded px-2 py-1 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">
                          <input v-model="item.qty" type="number" class="w-full min-w-[80px] text-sm border border-gray-300 rounded px-2 py-1 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">
                          <select v-model="item.uom" class="w-full min-w-[80px] text-sm border border-gray-300 rounded px-2 py-1 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                      ...
                      <template v-if="selectedCategory === 'packaging'">
                        <td class="px-4 py-3 whitespace-nowrap">
                          <input v-model="item.namaMaterial" type="text" class="w-full min-w-[150px] text-sm border border-gray-300 rounded px-2 py-1 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">
                          <input v-model="item.kodePM" type="text" class="w-full min-w-[120px] text-sm border border-gray-300 rounded px-2 py-1 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">
                          <input v-model="item.jumlahPermintaan" type="number" class="w-full min-w-[120px] text-sm border border-gray-300 rounded px-2 py-1 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </td>
                      </template>

                      <td class="px-4 py-3 whitespace-nowrap">
                        <button @click="removeItem(index)" class="bg-red-50 text-red-700 hover:bg-red-100 px-2 py-1 rounded text-xs font-medium transition-colors">Hapus</button>
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
              <button @click="closeModal" class="px-6 py-2 text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors">
                Batal
              </button>
              <button @click="submitRequest" :disabled="!isFormValid" class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors">
                Simpan & Submit
              </button>
            </div>
          </div>
