    ...
    <AppLayout title="Role & Permission Management">
        <div class="role-permission-management">
            ...
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Role & Permission Management</h1>
                <button 
            ...
            <div v-if="$page.props.flash?.success" class="bg-green-100 dark:bg-green-900 border border-green-400 dark:border-green-700 text-green-700 dark:text-green-200 px-4 py-3 rounded mb-4">
                {{ $page.props.flash.success }}
            </div>
            <div v-if="$page.props.flash?.error" class="bg-red-100 dark:bg-red-900 border border-red-400 dark:border-red-700 text-red-700 dark:text-red-200 px-4 py-3 rounded mb-4">
                {{ $page.props.flash.error }}
            </div>

            <!-- Tabel Daftar Role -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg dark:shadow-gray-900/50 overflow-hidden">
            ...
            <!-- Modal Tambah Role -->
            <div v-if="showAddRoleModal" class="fixed inset-0 bg-opacity-50 dark:bg-gray-900 dark:bg-opacity-75 backdrop-blur-sm flex items-center justify-center z-[9999]" style="background-color: rgba(43, 51, 63, 0.67);">
                <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md shadow-xl">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <button @click="closeAddRoleModal" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
            ...
            <div v-if="showPermissionModal" class="fixed inset-0 bg-opacity-50 dark:bg-gray-900 dark:bg-opacity-75 backdrop-blur-sm flex items-center justify-center z-[9999]" style="background-color: rgba(43, 51, 63, 0.67);">
                <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-xl">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Atur Permission - {{ selectedRole?.name }}</h3>
                        <button @click="closePermissionModal" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                            </svg>
                        </button>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
                                <label v-for="action in ['view', 'create', 'edit', 'delete', 'approve']" :key="`incoming-${action}`" class="flex items-center text-gray-700 dark:text-gray-300">
                                    <input 
                                        type="checkbox" 
                                        :checked="hasPermission('incoming', action)"
                                        @change="togglePermission('incoming', action, $event)"
                                        class="mr-2 rounded border-gray-300 dark:border-gray-600 text-blue-600 dark:text-blue-500 focus:ring-blue-500 dark:focus:ring-blue-400 dark:bg-gray-700"
                                    >
                        ...
                        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                <label v-for="action in ['view', 'input_qc_result', 'approve', 'reject']" :key="`qc-${action}`" class="flex items-center text-gray-700 dark:text-gray-300">
                                    <input 
                                        type="checkbox" 
                        ...
                        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                <label v-for="action in ['view', 'cetak_label', 'release', 'reject']" :key="`label-${action}`" class="flex items-center text-gray-700 dark:text-gray-300">
                                    <input 
                                        type="checkbox" 
                        ...
                        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                                <label v-for="action in ['view', 'kerjakan_to', 'cetak_slip']" :key="`putaway-${action}`" class="flex items-center text-gray-700 dark:text-gray-300">
                                    <input 
                                        type="checkbox" 
                        ...
                        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                <label v-for="action in ['view', 'create_request', 'approve_request', 'cetak_form']" :key="`reservation-${action}`" class="flex items-center text-gray-700 dark:text-gray-300">
                                    <input 
                                        type="checkbox" 
                        ...
                        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                                <label v-for="action in ['view', 'kerjakan_picking', 'cetak_picking_list']" :key="`picking-${action}`" class="flex items-center text-gray-700 dark:text-gray-300">
                                    <input 
                                        type="checkbox" 
                        ...
                        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                <label v-for="action in ['view', 'create_return', 'approve_return', 'cetak_slip']" :key="`return-${action}`" class="flex items-center text-gray-700 dark:text-gray-300">
                                    <input 
                                        type="checkbox" 
                        ...
                        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
                            ...
                            <div class="space-y-3">
                                <div v-for="module in ['sku_management', 'supplier_management', 'bin_management', 'user_management', 'role_management']" 
                                    ...
                                    <h5 class="font-medium text-gray-700 dark:text-gray-300 mb-2">{{ formatModuleName(module) }}</h5>
                                    <div class="grid grid-cols-2 md:grid-cols-5 gap-2">
                                        <label v-for="action in ['view', 'create', 'edit', 'delete', 'admin']" :key="`${module}-${action}`" class="flex items-center text-sm text-gray-700 dark:text-gray-300">
                                            <input 
                                                type="checkbox" 
                                                :checked="hasPermission('central_data', `${module}_${action}`)"
                                                @change="togglePermission('central_data', `${module}_${action}`, $event)"
                                                class="mr-1 rounded border-gray-300 dark:border-gray-600 text-blue-600 dark:text-blue-500 focus:ring-blue-500 dark:focus:ring-blue-400 dark:bg-gray-700"
                                            >
                                            {{ capitalizeFirst(action) }}
                                        </label>
                                    </div>
